...
async function deleteScript() {
  ...
  ScriptsDataService.deleteScript(scriptId.value)
    .then(async (res) => {
      if (res.status === 204) {
        await toggleModal();
        loader.hide();
        ToastService.showToast("Script was deleted successfully");
      }
    })
    .catch((err) => {
      toggleModal();
...
watch(
  () => bus.value.get("showScriptDeletingModal"),
  (val) => {
    toggleModal();
    scriptId.value = val[0].script_id;
  }
);
</script>
